...
my $hotc_home = defined $ENV{HOTC_HOME} ? $ENV{HOTC_HOME} : getcwd;
my $hotcd = $hotc_home . "/programs/hotcd/hotcd";

my $nodes = defined $ENV{HOTC_TEST_RING} ? $ENV{HOTC_TEST_RING} : "1";
my $pnodes = defined $ENV{HOTC_TEST_PRODUCERS} ? $ENV{HOTC_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$hotc_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of hotcd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"HOTC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = hotc::producer_plugin\n";
        print $cfg "plugin = hotc::chain_api_plugin\n";
