#include <HOTC/types/TypeParser.hpp>
#include <fstream>
#include <iomanip>
...
using std::string;
namespace HOTC_hotc { using types::SimpleSymbolTable; }
...
string generate_wren( const HOTC_hotc::types::Struct& s, HOTC_hotc::types::SimpleSymbolTable& symbols ) {
...
void generate_wren( HOTC_hotc::types::SimpleSymbolTable& ss, const char* outfile ) {
  //for( const auto& s : ss.order ) 
}

void generate_hpp( HOTC_hotc::types::SimpleSymbolTable& ss, const char* outfile ) {
   struct FakeField { std::string type; std::string name; };
   auto arrays_to_vectors = [](HOTC_hotc::types::Field f) {
   ...
   out << "#pragma once\n";
   out << "namespace HOTC_hotc { namespace types {\n";
   for( const auto& s : ss.order ) {
   ...
   for( const auto& s : ss.order ) {
      ...
      const auto& st = ss.structs[s];
      if( st.base.size() ) {
        out << "FC_REFLECT_DERIVED( HOTC_hotc::types::" << s << ", (HOTC_hotc::types::" << st.base <<"), ";
      } else  {
        out << "FC_REFLECT( HOTC_hotc::types::" <<  std::setw(33) << s << ", ";
      }
      for( const auto& f : st.fields ) {
         out <<"("<<f.name<<")";
      }
...
int  count_fields( HOTC_hotc::types::SimpleSymbolTable& ss, const HOTC_hotc::types::Struct& st ) {
   if( st.base.size() )
...
int main( int argc, char** argv ) {
  try {
     ...
     HOTC_hotc::types::SimpleSymbolTable ss;
     ss.parse(in);
