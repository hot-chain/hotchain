...
#include <hotc/account_history_plugin/account_transaction_history_object.hpp>
#include <hotc/account_history_plugin/public_key_history_object.hpp>
#include <hotc/account_history_plugin/transaction_history_object.hpp>
...
private:
   ...
   bool is_scope_relevant(const hotc::types::Vector<AccountName>& scope);
   ...
   for (const auto& cycle : block.cycles)
   {
      for (const auto& thread : cycle)
      {
         for (const auto& trx : thread.user_input)
         {
            ...
            {
               if (msg.code == config::HotcContractName)
               {
...
bool account_history_plugin_impl::is_scope_relevant(const hotc::types::Vector<AccountName>& scope)
{
