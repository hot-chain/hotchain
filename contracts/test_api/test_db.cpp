...
#include <hotclib/message.hpp>
#include <hotclib/db.h>
...
unsigned int store_set_in_table(uint64_t table_name)
{
  ...
  hotc::print("Store carol1 ", res, "\n");
  WASM_ASSERT(res > 0, "store carol1" );
  hotc::print("Store carol1 ", res, " ", carol1.id.name, " ", carol1.id.number, " ", carol1.price, " ", carol1.extra, "\n");
...
unsigned int test_db::key_i128i128_general() {
  ...
  //hotc::print("SECOND -> ", res, " ", tmp.id.name, " ", tmp.id.number, " ", tmp.price, " ", tmp.extra, "\n");
  WASM_ASSERT( res == sizeof(TestModel128x2) &&
               tmp.id.name == N(bob) &&
               tmp.id.number == 3 &&
               tmp.price == 4 &&
