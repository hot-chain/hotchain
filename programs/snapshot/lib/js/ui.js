...
(function(){
  ...
  const update_title_bar_progress = (stage = "") => {
    ...
    switch( stage ){
      ...
      case 'distribute':
        title = `HOTC | Snapshot ${debug.rates.percent_complete}% | ${debug.registry.accepted+debug.registry.rejected}/${debug.registry.total}`
      break
      case 'registry':
        title = `HOTC | Building Registry | ${output.registrants.length}`
      break
      case 'reclaimer':
        title = `HOTC | Reclaimable TXs | ${output.reclaimable.length}`
      break
      case 'complete':
        title = `HOTC | Snapshot Complete`
      break
      default:
        title = `HOTC | Welcome`
      break
