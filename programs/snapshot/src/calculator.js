...
const calculator = function(){
  ...
  debug.rejected = {
      ...
      ,key_is_malformed:                    0 // Starts with HOTC... but isn't a key
  ...
  debug.distribution = {
      ...
      ,$balance_reclaimable:                web3.toBigNumber(0) // crowdsale contract HOTC token balance + token contract HOTC token balance
      ,$balance_reclaimed:                  web3.toBigNumber(0) // crowdsale contract HOTC token balance + token contract HOTC token balance
  ...
  debug.output = function(){
   ...
   let _sanity = new AsciiTable('Sanity')
    ...
    if(output.reclaimed.length) {
      ...
      _reclaimed
          .setHeading(`ETH Address`, `HOTC Key`, `Amount`, `TX`)
      for( let tx of output.reclaimed ) {
        _reclaimed.addRow(tx.eth, tx.hotc, tx.amount.div(WAD).toFormat(4), `http://etherscan.io/tx/${tx.tx}`)
      }
