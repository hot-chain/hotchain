...
struct keypair {
  ...
  keypair ()
    : public_key("HOTC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"),
...
struct launcher_def {
  ...
  bool generate ();
  void define_nodes ();
  void write_config_file (hotcd_def &node);
  void make_ring ();
  void make_star ();
  void make_mesh ();
  void make_custom ();
  ...
  void prep_remote_config_dir (hotcd_def &node);
  void launch (hotcd_def &node, string &gts);
...
launcher_def::define_nodes () {
  ...
  for (int i = 0; i < total_nodes; i++) {
    ...
    network.nodes.insert (pair<string, hotcd_def>(name, node));

  }
}

void
launcher_def::write_config_file (hotcd_def &node) {
  bf::path filename;
  boost::system::error_code ec;
  ...
  if (!node.on_host()) {
    ...
    bf::path dpath = bf::path (node.hotc_root_dir) / node.data_dir / "config.ini";
...
void
launcher_def::prep_remote_config_dir (hotcd_def &node) {
  bf::path abs_data_dir = bf::path(node.hotc_root_dir) / node.data_dir;
  string add = abs_data_dir.string();
  string cmd = "cd " + node.hotc_root_dir;
  if (!do_ssh(cmd, node.hostname)) {
    cerr << "Unable to switch to path " << node.hotc_root_dir
...
void
launcher_def::launch (hotcd_def &node, string &gts) {
  bf::path dd = node.data_dir;
  bf::path reout = dd / "stdout.txt";
  bf::path reerr = dd / "stderr.txt";
  bf::path pidf  = dd / "hotcd.pid";

  node_rt_info info;
  info.remote = node.remote;

  string hotcdcmd = "programs/hotcd/hotcd ";
  if (skip_transaction_signatures) {
    hotcdcmd += "--skip-transaction-signatures ";
  }
  hotcdcmd += "--data-dir " + node.data_dir;
  if (gts.length()) {
    hotcdcmd += " --genesis-timestamp " + gts;
  }

  if (!node.on_host()) {
    string cmdl ("cd ");
    cmdl += node.hotc_root_dir + "; nohup " + hotcdcmd + " > "
    ...
    string cmd = "cd node.hotc_root_dir; kill -9 `cat " + pidf.string() + "`";
    format_ssh (cmd, node.hostname, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << hotcdcmd << endl;

    bp::child c(hotcdcmd, bp::std_out > reout, bp::std_err > reerr );
    ...
    if(!c.running()) {
      cout << "child not running after spawn " << hotcdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
FC_REFLECT( hotcd_def,
            (genesis)(remote)(ssh_identity)(ssh_args)(hotc_root_dir)(data_dir)
